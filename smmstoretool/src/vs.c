...
struct var_store_t vs_load(struct mem_range_t vs_data, bool auth_vars)
{
    ...
    while (var_hdr + var_hdr_size < vs_data.start + vs_data.length) {
        ...
        if (auth_vars) {
            const AUTHENTICATED_VARIABLE_HEADER *auth_hdr = (void *)var_data;
        ...
        var_hdr += HEADER_ALIGN(var_hdr_size + var.name_size + var.data_size);
        ...
        uint16_t *name = (void *)(var_data + var_hdr_size);
        var.name = xmalloc(var.name_size);
        memcpy(var.name, name, var.name_size);

        uint8_t *data = (void *)(var_data + var_hdr_size + var.name_size);
...
bool vs_store(struct var_store_t *vs, struct mem_range_t vs_data)
{
    ...
    for (struct var_t *var = vs->vars; var != NULL; var = var->next) {
        const size_t var_size = var_hdr_size + var->name_size + var->data_size;
        if (out_data + var_size > vs_data.start + vs_data.length) {
            fprintf(stderr, "Not enough space to serialize Variable Store.\n");
            return false;
        }
...
{
    size_t name_size;
    uint16_t *uchar_name = to_uchars(name, &name_size);
