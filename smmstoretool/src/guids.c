...
struct guid_alias_t known_guids[] = {
    {
        "coreboot",
        {
...
char * format_guid(const EFI_GUID *guid, bool use_alias)
{
    if (use_alias) {
        for (int i = 0; i < known_guid_count; ++i) {
            if (memcmp(&known_guids[i].guid, guid, sizeof(*guid)) == 0)
                return strdup(known_guids[i].alias);
        }
    }
...
{
    for (int i = 0; i < known_guid_count; ++i) {
        if (str_eq(known_guids[i].alias, str)) {
            *guid = known_guids[i].guid;
            return true;
        }
    }
